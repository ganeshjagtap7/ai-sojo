...
const limitFromEnv = (): number => {
  ...
  // set HV_LIMIT explicitly for a wider standalone scrape.
  if (process.env.HV_LIMIT === undefined) return Math.max(1, parseInt(process.env.SCRAPER_MAX_PAGES || '3', 10)) * 25;
